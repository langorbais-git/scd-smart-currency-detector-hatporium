        ...
        if($target_currency != $base_currency) {
            ...
            foreach( $order->get_items( array( 'line_item', 'tax', 'shipping', 'fee', 'coupon'  ) ) as $item_id => $item ) 
            {
                ...
                {				
					...
					// GET VENDOR ID
					$post_obj    = get_post( $item->legacy_values['product_id'] ); // The WP_Post object
					$vendor_id = $post_obj->post_author;

					$prod = get_post_meta( $item->legacy_values['product_id'], 'scd-product-price-in-vendor-currency' );

					if ( reset($prod)['currency'] == $target_currency){
						$item->legacy_values['line_subtotal'] = isset(reset($prod)['regular_price']) ? reset($prod)['regular_price'] * $item->legacy_values['quantity'] : $item->legacy_values['line_subtotal'];
						$item->legacy_values['line_total'] = isset(reset($prod)['regular_price']) ? reset($prod)['regular_price'] * $item->legacy_values['quantity'] : $item->legacy_values['line_total'];
						$item->legacy_values['currency'] = $target_currency;
						
						$product = $item->legacy_values['data'];

$product->set_sale_price( isset(reset($prod)['sale_price']) ? reset($prod)['sale_price'] : $item->legacy_values['data']->sale_price );
$product->set_price( isset(reset($prod)['regular_price']) ? reset($prod)['regular_price'] : $item->legacy_values['data']->price );
$product->set_regular_price( isset(reset($prod)['regular_price']) ? reset($prod)['regular_price'] : $item->legacy_values['data']->regular_price );
						
					}else{

						$item->legacy_values['line_subtotal'] = isset(reset($prod)['regular_price']) ? scd_function_convert_subtotal((float) reset($prod)['regular_price'] * $item->legacy_values['quantity'], reset($prod)['currency'], $target_currency ) : scd_function_convert_subtotal((float) $item->legacy_values['line_subtotal'], reset($prod)['currency'], $target_currency );
						$item->legacy_values['line_total'] = isset(reset($prod)['regular_price']) ? scd_function_convert_subtotal((float) reset($prod)['regular_price'] * $item->legacy_values['quantity'], reset($prod)['currency'], $target_currency ) : scd_function_convert_subtotal((float) $item->legacy_values['line_total'], reset($prod)['currency'], $target_currency );
						$item->legacy_values['currency'] = $target_currency;
						
						$product = $item->legacy_values['data'];

$product->set_sale_price(isset(reset($prod)['sale_price']) ? scd_function_convert_subtotal((float) reset($prod)['sale_price'], reset($prod)['currency'], $target_currency ) : scd_function_convert_subtotal((float) $item->legacy_values['data']->sale_price, reset($prod)['currency'], $target_currency ));
$product->set_price( isset(reset($prod)['regular_price']) ? scd_function_convert_subtotal((float) reset($prod)['regular_price'], reset($prod)['currency'], $target_currency ) : scd_function_convert_subtotal((float) $item->legacy_values['data']->price, reset($prod)['currency'], $target_currency ) );
$product->set_regular_price( isset(reset($prod)['regular_price']) ? scd_function_convert_subtotal((float) reset($prod)['regular_price'], reset($prod)['currency'], $target_currency ) : scd_function_convert_subtotal((float) $item->legacy_values['data']->regular_price, reset($prod)['currency'], $target_currency ) );


					}
					
				$product->apply_changes();
				$item->legacy_values['data'] = $product;
         ...
         */               // No custom price per currency is specified for this product.
         ...
         //           }
                ...
                elseif( $item['type'] === 'shipping' )
                {
					...
					foreach( $item->get_meta_data() as $item_shipping_id => $shipping ) 
					{
						
						// dump($shipping);
						
						$current_data_value = $shipping->__get('value');
						$total_shipping_cost = 0;
						$total_product_cost = 0;
						
						foreach( $current_data_value['items'] as $current_data_value_item_key => $current_data_value_item ) 
						{
							// GET VENDOR ID
							$post_obj    = get_post( $current_data_value_item['product_id'] ); // The WP_Post object
							$vendor_id = $post_obj->post_author;
			
							$prod = get_post_meta( $current_data_value_item['product_id'], 'scd-product-price-in-vendor-currency' );
							if ( reset($prod)['currency'] == $target_currency){ // WHEN BASE CURRENCY != TARGET CURRENCY AND TARGET CURRENCY == VENDOR CURRENCY
																				// SET THE VALUES IN VENDOR CURRENCY
								...
								if ( $user_meta ) {
									...
									/* var_dump($country_filter);
									var_dump($country_count); */

									if($country_count >= 1){
											$state_filter= find_in_two_dimension_array($country_filter, "state", $order->shipping_state);
											$state_count = count($state_filter);
																							
											/* var_dump( $order->shipping_state);
											var_dump($state_filter);
											var_dump($state_count); */


											if ( $state_count == 1 ) {
											
												$postcode_filter= find_in_two_dimension_array($state_filter, "postcode", $order->shipping_postcode);
												$postcode_count = count($postcode_filter);
																								
												//var_dump($postcode_filter);
												//var_dump($postcode_count);	
								...
								$current_data_value['items'][$current_data_value_item_key]['total_product_cost'] = isset(reset($prod)['regular_price']) ? reset($prod)['regular_price'] * WC()->cart->cart_contents[$current_data_value_item_key]['quantity'] : $current_data_value['items'][$current_data_value_item_key]['total_product_cost'];
								$total_product_cost += $current_data_value['items'][$current_data_value_item_key]['total_product_cost'];
							
							}else{ // WHEN BASE CURRENCY != TARGET CURRENCY AND TARGET CURRENCY != VENDOR CURRENCY
									// CONVERT THE VALUES FROM VENDOR CURRENCY TO TARGET CURRENCY
								$user_meta = reset ( get_user_meta($vendor_id, '_wcv_shipping_rates' ));// GET THE WCV SHIPPING TABLE RATES
						...
						$shipping->__set('value', $current_data_value);
...
// dump($changes);
// dump($order->changes);
// $order->changes['shipping_total'] = 60;
// dump($order->changes['shipping_total']);
// die;
        }
...
//apply_filters( 'wc_stripe_generate_create_intent_request', $request, $order, $prepared_source );

add_filter('wc_stripe_generate_create_intent_request', 'scd_wc_stripe_generate_create_intent_request', 999, 3); 

function scd_wc_stripe_generate_create_intent_request ($request, $order, $prepared_source){
	
	$amount = $request['amount'] ; 
	
	$tab_curr = array ( "BIF","CLP","DJF","GNF","JPY","KMF","KRW","MGA","PYG","RWF","UGX","VND","VUV","XAF","XOF","XPF");
	
    $tar_curr = scd_get_target_currency();
	
	if(in_array($tar_curr, $tab_curr)){
	   if ($amount != 0 ){
       $amount = $amount/100 ;
...
function find_in_two_dimension_array($products, $field, $value)
{										
	$result = [];
