...
add_action('init',function(){
    ...
    if (!get_option("scd_free_cancel") && is_admin() && in_array('scd-smart-currency-detector/index.php', apply_filters('active_plugins', get_option('active_plugins'))) ) {
        $current_page = filter_input( INPUT_GET, 'page' );
            require_once 'scd_for_marketplace_install.php';
    }
...
function scd_add_scripts_to_post() {
   ...
   wp_enqueue_script('ch_scd_fetch', trailingslashit(plugins_url("", __FILE__)) . "js/scd_fetchdata.js", array('ch_scd_maps_js','ch_scd_defaultdata_js'), '5.5.3', true);
	...
	wp_localize_script('ch_scd_postready', 'scd_ajax', $variable_to_js);
